...
// proxy has an idle write timeout on inbound request bodies; any async work
// (auth, DB queries) that runs before the body is consumed causes backpressure
// that makes Fastly drop the connection, and the browser sees ERR_TIMED_OUT.
createServer(async (req: IncomingMessage, res: ServerResponse) => {
  // Step 1 — drain body immediately so Fastly never hits its write timeout.
  const chunks: Buffer[] = [];
  try {
  ...
  // x-forwarded-proto may be comma-separated (e.g. "https, https") when
  // multiple proxies are in the chain; take the first value only.
  ...
  for (const [key, val] of Object.entries(req.headers)) {
    ...
    if (key.toLowerCase() === "host") continue; // already encoded in the URL
