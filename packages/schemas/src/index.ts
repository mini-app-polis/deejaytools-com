...
export const createCheckinBodySchema = z
  ...
  .refine(
    (b) => {
      const entityCount = [b.entityPairId, b.entityManagedPartnershipId].filter(Boolean).length;
    ...
    {
      message:
        "Exactly one of entityPairId or entityManagedPartnershipId must be provided (or none when checking in on behalf)",
    }
  );
...
export const ApiMyCheckinSchema = z.object({
  ...
  divisionName: z.string(),
  entityPairId: z.string().nullable().optional(),
  entitySoloUserId: z.string().nullable().optional(),
  entityManagedPartnershipId: z.string().nullable().optional(),
  entityLabel: z.string(),
